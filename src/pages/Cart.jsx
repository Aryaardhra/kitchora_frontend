...
const Cart = () => {
    ...
    const getCart = () => {
        let tempArray = [];
        for (const list in cartItems){
            const product = products.find((item) => item._id === list)
            product.quantity = cartItems[list]
    ...
    return products.length > 0 && cartItems ? (
        <div className="flex flex-col md:flex-row mt-16">
            <div className='flex-1 max-w-4xl'>
                ...
                className="group cursor-pointer flex items-center mt-8 gap-2 text-text font-medium">
            ...
            <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
                ...
                <div className="mb-6">
                    ...
                    <div className="relative flex justify-between items-start mt-2">
                        ...
                        <button onClick={() => setShowAddress(!showAddress)} className="text-text hover:underline cursor-pointer">
                            Change
                        </button>
                        {showAddress && (
                            <div className="absolute top-12 py-1 bg-white border border-gray-300 text-sm w-full">
                                {addresses.map((addresses, index) => (
                                    <p  key={index} onClick={() => {setSelectedAddress(addresses); setShowAddress(false)}} 
                                    className="text-gray-500 p-2 hover:bg-gray-100">
                                        {addresses.street}, {addresses.city}, {addresses.state}, {addresses.country}
                                    </p>
                                ))}
                                <p onClick={()=> navigate("/add-address")}
                                className="text-text text-center cursor-pointer p-2 hover:bg-secondary">Add Address</p>
                            </div>
                        )}
                    </div>
                ...
                <div className="text-gray-500 mt-4 space-y-2">
                    ...
                    <p className="flex justify-between">
                        <span>Shipping Fee</span><span className="text-subtext">Free</span>
                    </p>
                    <p className="flex justify-between">
                ...
                <button onClick={placeOrder} className="w-full py-3 mt-6 cursor-pointer bg-primary/75 text-white font-medium hover:bg-secondary/95 transition">
